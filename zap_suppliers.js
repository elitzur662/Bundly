...
const DELAY_MS  = 1400;    // polite pause between requests
...
async function fetchCategoryPage(sog, pageIdx = 1) {
  const url = `${ZAP_BASE}/models.aspx?sog=${sog}&orderby=2${pageIdx > 1 ? `&Pageindex=${pageIdx}` : ''}`;
  try {
    const resp = await axios.get(url, axCfg());
    const html = decode1255(resp.data);
    return extractModelIds(html);
  } catch (e) {
    process.stderr.write(`  ⚠ cat page ${sog} p${pageIdx}: ${e.message}\n`);
    return [];
  }
}
...
async function fetchModelStores(modelId) {
  const url = `${ZAP_BASE}/model.aspx?modelid=${modelId}&orderby=1`;
  try {
    const resp = await axios.get(url, axCfg({ headers: zapHeaders(`${ZAP_BASE}/models.aspx`) }));
    const html = decode1255(resp.data);
    return parseModelPageStores(html, modelId);
  } catch (e) {
    process.stderr.write(`  ⚠ model ${modelId}: ${e.message}\n`);
    return [];
  }
}
